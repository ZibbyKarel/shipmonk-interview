...
public class ExchangeRatesService {
  ...
  public ExchangeRatesService(ExchangeRateSnapshotRepository repository,
  ...
  public RatesResponse getRatesForDay(LocalDate date) {
    ...
    return repository.findByDate(date).map(snapshot -> {
  ...
  private RatesResponse fetchAndCache(LocalDate date) {
    ...
    ExchangeRateSnapshot snapshot = new ExchangeRateSnapshot(date, rates.base(), rates.timestamp(),
        rates.rates(), LocalDateTime.now());
    ...
    try {
      ...
      return repository.findByDate(date).map(this::toResponse).orElseThrow(
          () -> new ProviderException("Concurrent write conflict for date: " + date, e));
    }
  ...
  private RatesResponse toResponse(ExchangeRateSnapshot snapshot) {
    return new RatesResponse(snapshot.getDate().toString(), snapshot.getTimestamp(),
        snapshot.getBase(), snapshot.getRates());
  }
}
