...
public class FixerExchangeRateProvider implements ExchangeRateProvider {
  ...
  public FixerExchangeRateProvider(RestTemplate restTemplate,
      @Value("${fixer.api.base-url}") String baseUrl, @Value("${fixer.api.key}") String apiKey) {
  ...
  private FixerRatesResponse callFixer(LocalDate date) {
    String url = UriComponentsBuilder.fromHttpUrl(baseUrl + "/{date}")
        .queryParam("access_key", apiKey).buildAndExpand(date.toString()).toUriString();
    ...
    try {
      ...
      throw new ProviderException(
          "fixer.io client error " + e.getStatusCode() + ": " + e.getMessage(), e);
    } catch (HttpServerErrorException e) {
      log.warn("fixer.io server error {} for date={}", e.getStatusCode(), date);
      throw new TransientProviderException(
          "fixer.io server error " + e.getStatusCode() + ": " + e.getMessage(), e);
