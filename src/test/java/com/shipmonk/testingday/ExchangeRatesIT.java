...
class ExchangeRatesIT {
  ...
  void cacheMiss_fetchesFromFixer_persistsRebasedRatesToDb_returns200() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/rates/2022-06-20")).andExpect(status().isOk())
        ...
        .andExpect(jsonPath("$.base").value("USD")).andExpect(jsonPath("$.rates.USD").value(1))
  ...
  void cacheHit_secondCall_doesNotHitFixer_returnsSameBody() throws Exception {
    ...
    MvcResult first =
        mockMvc.perform(get("/api/v1/rates/2022-06-20")).andExpect(status().isOk()).andReturn();
    ...
    mockServer.verify();
    JSONAssert.assertEquals(first.getResponse().getContentAsString(),
        second.getResponse().getContentAsString(), JSONCompareMode.STRICT);
  }

  @Test
  void malformedDate_returns400_withErrorBody() throws Exception {
    mockMvc.perform(get("/api/v1/rates/not-a-date")).andExpect(status().isBadRequest())
  ...
  void futureDate_returns400_withInvalidDateExceptionMessage() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/rates/" + future)).andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error.code").value(400))
  ...
  void fixerCode106_noRatesForDate_returns404_andDoesNotPersist() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/rates/1900-01-01")).andExpect(status().isNotFound())
        .andExpect(jsonPath("$.success").value(false))
  ...
  void fixerCode104_quotaExceeded_returns502_doesNotRetry() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/rates/2022-06-20")).andExpect(status().isBadGateway())
        .andExpect(jsonPath("$.success").value(false))
  ...
  void transient429ThenSuccess_retriesAndReturns200() throws Exception {
    ...
    mockServer.expect(ExpectedCount.once(), requestTo(fixerUrl(date)))
        .andExpect(method(HttpMethod.GET)).andRespond(withStatus(HttpStatus.TOO_MANY_REQUESTS));
    mockServer.expect(ExpectedCount.once(), requestTo(fixerUrl(date)))
  ...
  private void expectFixerCall(LocalDate date, String responseBodyJson) {
    mockServer.expect(requestTo(fixerUrl(date))).andExpect(method(HttpMethod.GET))
