...
class ExchangeRatesControllerTest {
    ...
    void malformedDate_returns400_withErrorBody() throws Exception {
        mockMvc.perform(get("/api/v1/rates/not-a-date"))
                ...
                .andExpect(jsonPath("$.error.info").value(org.hamcrest.Matchers.containsString("Invalid date format")));
    }

    @Test
    void futureDate_returns400_fromService() throws Exception {
        when(exchangeRatesService.getRatesForDay(any()))
                .thenThrow(new InvalidDateException("Cannot retrieve rates for a future date: 2099-01-01"));

        mockMvc.perform(get("/api/v1/rates/2099-01-01"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.error.code").value(400))
                .andExpect(jsonPath("$.error.info").value(org.hamcrest.Matchers.containsString("future")));
    }

    @Test
    void notFound_returns404() throws Exception {
        when(exchangeRatesService.getRatesForDay(any()))
                .thenThrow(new RatesNotFoundException("No exchange rates available for date: 1900-01-01"));
