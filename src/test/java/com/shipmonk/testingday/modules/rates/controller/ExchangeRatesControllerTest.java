...
class ExchangeRatesControllerTest {
  ...
  void happyPath_returnsRatesJsonInFixerShape() throws Exception {
    RatesResponse stub = new RatesResponse("2022-06-20", 1655769599L, "USD", Map.of("USD",
        BigDecimal.ONE, "EUR", new BigDecimal("0.952381"), "GBP", new BigDecimal("0.809524")));
  ...
  void malformedDate_returns400_withErrorBody() throws Exception {
    mockMvc.perform(get("/api/v1/rates/not-a-date")).andExpect(status().isBadRequest())
        ...
        .andExpect(jsonPath("$.error.code").value(400)).andExpect(jsonPath("$.error.info")
